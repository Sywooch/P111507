...
class VideoURL {
    ...
    public function getThumbnailYoutubeByURL($url, $size = 'hqdefault') {
        if (false !== ($id = $this->getYoutubeId($url))) {
             ...
             */
            if (empty($size) || !in_array($size, ['0', '1', '2', '3', 'default', 'hqdefault', 'mqdefault', 'sddefault', 'maxresdefault'])) {
                $size = 'mqdefault';
            }
    ...
    public function getVideoThumbnailByUrl($url, $format = 'small') {
        ...
        } elseif (false !== ($id = $this->getDailyMotionId($url))) {
