...
function url_exists($url){
	...
	if ($result !== false)
	{
		...
		if ($statusCode == 200)
		{
			$arrayResult['newUrl'] = $url;
			$arrayResult['status'] = 1;
			$arrayResult['status_code_url'] = $statusCode;
		}
		else if($statusCode == 301 || $statusCode == 302)
		{
			$newUrl = get_redirect_target($url);
			//CURL NEWURL 
			$arrayResult = array();
			$cNewUrl = curl_init($newUrl);
			curl_setopt($cNewUrl, CURLOPT_NOBODY, true);
			$resultNew = curl_exec($cNewUrl);
			$statusCode = curl_getinfo($cNewUrl, CURLINFO_HTTP_CODE);
			if ($statusCode == 200)
			{
				$arrayResult['newUrl'] = $newUrl;
				$arrayResult['status'] = 1;
				$arrayResult['status_code_url'] = $statusCode;
			}
			else
	...
	{
		$arrayResult['status'] = 0;
		$arrayResult['status_code_url'] = "404 Curl exit";
	}
	return $arrayResult;
}
function file_get_html($url,$id_cate ="", $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
	//$urlExit = url_exists($url);
	//var_dump($urlExit);
	/*
	if(isset($id_cate))
	{
		$idcate = intval($id_cate);
		$status_code_url = $urlExit['status_code_url'];
		$sqlUpdate = "UPDATE tbl_software_category SET status_code_url = '".$status_code_url."' WHERE id = ".$idcate;
