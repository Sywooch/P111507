...
class CRestFull
{
    ...
    */
    private function __construct()
    {
        ...
        curl_setopt($this->curl, CURLOPT_CONNECTTIMEOUT, 120);   // timeout on connect
        curl_setopt($this->curl, CURLOPT_TIMEOUT, 120);          // timeout on response 
        curl_setopt($this->curl, CURLOPT_SSL_VERIFYPEER, false); // https
        curl_setopt($this->curl, CURLOPT_MAXREDIRS, 3);
        curl_setopt($this->curl, CURLOPT_ENCODING, 'UTF-8');
        curl_setopt($this->curl, CURLOPT_USERAGENT, (isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : getenv('HTTP_USER_AGENT')));
    }
    ...
    */
    private function treatResponse($r)
    {
        ...
        preg_match("@HTTP/1.[0-1] ([0-9]{3}) ([a-zA-Z ]+)@", $parts[0],
            $reg);                                                              // This extracts the response header Code and Message
    ...
    */
    ...
    {
        return self::createClient($url)->setParameters($body)->setMethod(
            $method)->setCredentials($user, $pwd)->setContentType($contentType)->execute()->close();
    }
}
?>
