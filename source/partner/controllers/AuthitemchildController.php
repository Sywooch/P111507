 ...
 */
class AuthitemchildController extends BackendController
{
    /**
	...
	public function actionAuthItem()
	{
		...
		$arrayAuthItem 				= AuthItem::find()->where("type = 2")->orderBy('name asc')->all();
		$arrayAuthManage 			= AuthItem::find()->where("type = 1")->orderBy('type asc, name asc')->all();
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) 
		{
            ...
            if (count($arr_child) > 0) 
			{
                ...
                if (count($arr_err) > 0) {
                    $err = 'Thêm mới phân quyền thất bại cho Role ' . $model->parent . ': ' . implode(', ', $arr_err);
                    $model->addError('child', $err);
                } else {
                    $success = 'Thêm mới phân quyền thành công cho Role ' . $model->parent;
                }
            } else {
                $err = 'Vui lòng chọn Child (Role | Permission)';
                $model->addError('child', $err);
            }            
        }
